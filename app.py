"""
Hospital Project — Deployment App
-----------------------------------
Streamlit app that deploys the EDA + ML work from hospital_eda_ml.ipynb.

Setup:
    1. Run hospital_eda_ml.ipynb once (creates hospital_model.pkl + model_features.pkl)
    2. pip install -r requirements.txt
    3. streamlit run app.py

Deploy free: push this folder to your hospital_database_project GitHub repo,
...
with tab_overview:
    ...
    st.markdown("""
    This app deploys the pipeline built in `hospital_eda_ml.ipynb`:
    the same three CSVs from the repo → cleaned → explored → used to train a model
    that predicts whether a booked appointment will be **Completed** or stay **Pending**.
    """)
...
with tab_eda:
    ...
    with col4:
        ...
        monthly = appointments.groupby(
            appointments["appointment_date"].dt.to_period("M")
        ).size()
    ...
    workload = (
        master.groupby(["doctor_name", "specialization", "experience_years"])
        .size()
...
with tab_predict:
    ...
    if model is None:
        st.warning(
            "No trained model found yet. Run `hospital_eda_ml.ipynb` once — "
            "Section 6 saves `hospital_model.pkl`, which this tab loads."
        )
    else:
        st.caption(
            "This uses the Random Forest pipeline trained in the notebook "
            "(patient age/gender, doctor specialization/experience, and appointment timing)."
        )
        col1, col2 = st.columns(2)
        with col1:
            ...
            specialization = st.selectbox(
                "Doctor specialization", sorted(doctors["specialization"].unique())
            )
        with col2:
        ...
        if st.button("Predict"):
            ...
            st.caption(
                "This is a small-sample demo model (71 labeled appointments) — treat the "
                "probability as illustrative, not a clinical or operational decision tool."
            )
